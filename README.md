...
```
> **복원 실패 후 복구**: 복원 유닛은 `Requires=` 로 본체를 막는다(fail-closed). 원인(예: 자격증명)을
> 고친 뒤 본체를 다시 띄우려면 실패 상태를 먼저 지워야 한다:
> `sudo systemctl reset-failed <svc>-s3-restore && sudo systemctl start <svc>.service`.
> 유닛 실패 시 Slack 알림이 오며(`slack-alert@`), 웹훅 시크릿이 비어 있으면 알림은 조용히 생략된다.
...
이 저장소만으로 빈 디스크에서 서버를 재구성하는 절차. `nixos/disk-config.nix`(disko)가 파티션을,
...
3. **디스크 파티션 + 설치** — disko + nixos-install을 한 번에:
   ```bash
   # 디스크 장치 확인 후(예: /dev/nvme0n1) 실행. disko 는 flake.lock 에 핀된 버전 사용(.#disko).
   cd /tmp/nixos-server
   ...
   ```
   > 디스크 장치명은 `nixos/disk-config.nix`의 `/dev/nvme0n1` 기준. 다르면 먼저 맞춘다.
4. **재부팅** 후 SSH 접속(공개키는 선언적으로 이미 등록됨. `configuration.nix` 참고).
   > 이 시점엔 시크릿 복호화가 아직 안 돼 headscale·aliced·gitea·DDNS·백업이 **기동 실패 상태**다. 정상이다 — 5단계로 해결.
5. **⚠️ agenix 시크릿 재암호화 (필수, 재부팅 후에 한다)**: 시크릿(`.age`)은 **서버의 SSH 호스트키**로
   암호화돼 있는데, 재설치하면 호스트키가 **첫 부팅 때 새로 생성**된다(그래서 이 단계는 반드시 재부팅
   *후*에 — 설치 미디어의 호스트키가 아니라 새 서버의 것을 써야 한다). 새 호스트키를 수신자로 추가해
   재암호화해야 하며, **새 호스트키는 아직 수신자가 아니라 서버 스스로는 복호화 못 하므로 기존 수신자
   키를 가진 맥북에서 rekey** 한다:
   ```bash
   # (서버, 재부팅 후) 저장소 클론 + 새 호스트키 공개키 확인
   git clone <이 저장소> /home/alicek106/nixos-server
   sudo cat /etc/ssh/ssh_host_ed25519_key.pub          # ← 이 값을 맥북으로 전달
   # (맥북 = 기존 수신자) secrets/secrets.nix 의 `server = "ssh-ed25519 ..."` 를 위 값으로 교체 후:
   cd nixos/secrets
   agenix -r -i ~/.ssh/id_ed25519      # 맥북 alice 키로 모든 .age 재암호화 (-i 는 실제 키 경로에 맞게)
   git commit -am "rekey secrets to new host key" && git push
   # (서버) pull 후 rebuild → 이제 새 호스트키로 복호화되어 서비스 기동
   cd /home/alicek106/nixos-server && git pull
   sudo nixos-rebuild switch --flake .#nixos-alicek106
   ```
6. **설치 후 수동 단계**는 아래 "재현 불가능한 수동 설정"을 따른다 (Claude 인증, Slack 웹훅 등).
...
### 3. Slack 알림 (Webhook — agenix)
두 경로가 같은 웹훅 시크릿을 쓴다: (a) Claude 완료/입력대기 알림(`home/claude-code.nix` Notification 훅),
(b) systemd 유닛 실패 알림(`modules/services/slack-alert.nix`, 백업/복원/DDNS 의 `OnFailure`).
웹훅 URL 은 **agenix 시크릿**으로 관리한다 — repo 밖 수동 파일이 아니다(재현적):
- `nixos/secrets/slack-webhook.age` (env-file 형식, 키 이름 `SLACK_WEBHOOK_URL`).
  편집: `cd nixos/secrets && sudo EDITOR=vim agenix -e slack-webhook.age -i /etc/ssh/ssh_host_ed25519_key`
  내용 예:
  ```
  SLACK_WEBHOOK_URL=https://hooks.slack.com/services/T00000000/B00000000/XXXXXXXXXXXX
  ```
- 복호화 결과는 `/run/agenix/slack-webhook` (owner=alicek106 → 유저 훅도 읽음). 시크릿이 없거나 비면 조용히 무동작.
...
세 워크로드 모두 이제 **비어 있을 때 S3 에서 자동 복원**된다(SoT = S3). 재설치 시 밀고 rebuild 만 하면 복구된다.

> **⚠️ 최종 안전망 = S3 버킷 버저닝 (AWS 콘솔에서 1회 수동 설정 — 강력 권장).** 백업은 매일 같은 키를
> 덮어쓴다(`headscale/…tar.gz`, `gitea/…tar.gz`). nix 쪽은 "빈 상태면 업로드 거부 + 업로드 전 `gzip -t`
> 자기검증"으로 방어하지만, 그래도 논리적 손상이 정상처럼 백업될 여지는 남는다. **버킷 버저닝을 켜두면**
> 나쁜 덮어쓰기도 이전 버전으로 롤백할 수 있다. 추가로 실패 시엔 systemd `OnFailure` → Slack 알림이 온다.
> (관련: IAM 을 최소권한으로 좁힐 때 복원 가드가 head-object 404 판별에 의존하므로 `s3:ListBucket` 유지 필요.)
- **aliced** (`/var/lib/aliced/data`): 빈 상태로 뜨면 앱 코드가 **S3 에서 자동 복원**.
  백업 위치 `s3://alicek106-backup/aliced/` (버킷·prefix 는 `aliced.nix` 의 `S3_BUCKET_NAME`/`S3_BACKUP_PREFIX`).
- **gitea** (`/var/lib/gitea`): `backup.nix` 의 `gitea-s3-backup`(매일) 이 repos+sqlite DB+conf 를
  `s3://alicek106-backup/gitea/gitea-backup.tar.gz` 로 백업하고, `gitea-s3-restore` 가 컨테이너 기동 전
  상태가 비어 있으면 자동 복원(혼합 소유권을 root+`--numeric-owner` 로 보존). **수동 조치 불필요.**
  - 수동 백업/복원(fallback): `sudo systemctl start gitea-s3-backup` / 복원은 `/var/lib/gitea` 를 비우고 재부팅(또는
    `sudo systemctl start gitea-s3-restore`). ⚠️ `gitea/conf/app.ini`(SECRET_KEY 등)는 DB 와 짝이라 항상 함께 복원돼야 함.
  - (최초 AWS 이전분은 옛 EBS tarball 을 `/var/lib/gitea` 에 한 번 풀어 부트스트랩했고, 이후 SoT 는 위 S3 백업이다.)
