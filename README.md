...
**상태 복원 (SoT = S3):** tailnet 원장은 headscale DB 라, 재설치 시 복원 없이 rebuild 하면 **전 노드 재등록**·**서버 IP 변동**이 발생한다.
- 매일 `headscale-s3-backup` 타이머가 `db.sqlite`+`noise_private.key` 를 `s3://alicek106-backup/headscale/headscale-backup.tar.gz` 로 백업.
- 복원: headscale 정지 → `aws s3 cp s3://alicek106-backup/headscale/headscale-backup.tar.gz - | tar -xz -C /var/lib/headscale` → rebuild.
  (`noise_private.key` = 컨트롤 서버 정체성. 분실 시 전 노드 재등록 필요.)
- 서버 tailnet IP 고정: 컨테이너가 `100.64.0.2` 에 바인딩하므로 복원 후 이 값이 유지돼야 한다.
  새로 등록됐다면 `sudo headscale nodes set-ip <node-id> 100.64.0.2` 로 맞춘다.
