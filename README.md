...
**상태 복원 (SoT = S3, ⚠️ 자동 아님):** aliced 는 앱 코드가 `/data` 가 비면 S3 에서 **스스로 복원**하지만,
headscale 은 `backup.nix` 에 **업로드만** 있고 복원 경로가 없다. 즉 **서버를 새로 밀면 headscale 상태는 자동으로 안 온다.**
복원 없이 그냥 rebuild 하면 headscale 이 **새 `db.sqlite`+`noise_private.key`(새 서버 정체성·새 IP)** 를 만들어
**전 노드 재등록**되고 aliced/gitea 의 `100.64.0.2` 바인딩이 깨진다. 그래서 재설치 시 아래 순서를 반드시 지킨다:

- 매일 `headscale-s3-backup` 타이머가 `db.sqlite`+`noise_private.key` 를 `s3://alicek106-backup/headscale/headscale-backup.tar.gz` 로 백업.
- **재설치 후 복원 순서** (시스템이 이미 부팅돼 headscale 이 throwaway 상태를 만든 상태에서):
  ```bash
  sudo systemctl stop headscale                       # 1. 새로 생성된 throwaway 상태 사용 중단
  ...
  sudo systemctl start headscale                       # 3. 원래 원장/정체성으로 기동
  ```
