...
4. **⚠️ agenix 시크릿 재암호화 (필수)**: 시크릿(`.age`)은 **이 서버의 SSH 호스트키**로 암호화돼 있는데,
   새로 설치하면 호스트키가 **새로 생성**되어 기존 시크릿을 복호화할 수 없다. 그러면 headscale·aliced·
   gitea·DDNS·백업이 전부 기동 실패한다. 새 호스트키를 수신자로 추가해 재암호화해야 한다.
   순서상 **새 호스트키는 아직 수신자가 아니라 서버 스스로는 복호화 못 하므로, 기존 수신자 키를 가진
   맥북에서 rekey** 한다:
   ```bash
   # (서버) 새 호스트키 공개키 확인
   sudo cat /etc/ssh/ssh_host_ed25519_key.pub
   # (맥북 = 기존 수신자) secrets/secrets.nix 의 `server = "ssh-ed25519 ..."` 를 위 값으로 교체 후
   cd nixos/secrets && agenix -r          # 모든 .age 를 새 수신자 집합으로 재암호화
   git commit -am "rekey secrets to new host key" && git push
   # (서버) 최신 커밋 pull 후 rebuild → 이제 호스트키로 복호화 가능
   ```
5. **재부팅** 후 SSH 접속(공개키는 선언적으로 이미 등록됨. `configuration.nix` 참고).
6. **설치 후 수동 단계**는 아래 "재현 불가능한 수동 설정"을 따른다 (Claude 인증 등).
